...
- ✅ `FrameExtractionTool/FrameExtractionTool.entitlements`
...
Xcode creates default Share Extension files. Replace them with our custom implementation:

1. In the Project Navigator, find the `ShareExtension` folder Xcode just created
2. **Delete** the following generated files (Move to Trash):
   - `ShareViewController.swift` (generated by Xcode)
   - `Info.plist` (if generated separately)
   - Any storyboard files
3. **Add** our custom files to the ShareExtension target:
   - Right-click on `ShareExtension` folder → "Add Files to FrameExtractionTool..."
   - Navigate to `ShareExtension/ShareViewController.swift` → Add
   - Navigate to `ShareExtension/Info.plist` → Add (uncheck "Copy items if needed")
   - Navigate to `ShareExtension/ShareExtension.entitlements` → Add
